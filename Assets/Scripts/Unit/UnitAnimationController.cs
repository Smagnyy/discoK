...
public class UnitAnimationController : MonoBehaviour
{
    ...
    public void StartAnim(string animName)
    {
        if(!isLocked)
        {
            ...
            currentAnim = StartCoroutine(AnimCoroutine(animName));
        }
    ...
    public IEnumerator AnimCoroutine(string _animName)
    {
    ...
    public IEnumerator AnimCoroutineWithChange(string _animName, string _nextAnimName)
    {
        //if(animator.GetCurrentAnimatorStateInfo(0).IsName(_animName))
        //     animator.Play("Empty");
        animator.Play(_animName, -1, 0);
